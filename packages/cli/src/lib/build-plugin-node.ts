...
${appEntry ? '' : "import { createNodeWebSocketTransport } from '@flue/runtime/node';"}
...
function normalizeBuiltModules(agentModules, workflowModules) {
  ...
  if (${appEntry ? 'true' : 'false'} && (Object.keys(websocketAgentHandlers).length > 0 || Object.keys(websocketWorkflowHandlers).length > 0)) {
    throw new Error('[flue] websocket() on the Node target currently requires the generated default app. Custom app.ts WebSocket mounting is not yet supported.');
  }
...
${appEntry ? '' : 'websocketTransport.attach(server);'}
...
async function stop() {
  ${appEntry ? '' : 'await websocketTransport.close();'}
  server.close();
  process.exit(0);
}
